...
Ext.define('Optima5.Modules.Spec.RsiRecouveo.CfgParamTree',{
		...
		if( this.cfgParam_id && this.cfgParam_id.indexOf('ADR_')===0 && this.accountRecord ) {
			...
			Ext.Array.each( Optima5.Modules.Spec.RsiRecouveo.HelperCache.getUserAll(), function(userRow) {
				...
				rootChildren.push({
					...
					nodeText: userRow.user_fullname,
					leaf: true
				});
			}) ;
			rootNode = {
				...
				nodeText: '<b>'+'Affectation'+'</b>',
				expanded: true
			}
			this.allValues=true ;
		} else if( this.cfgParam_id && this.cfgParam_id=='EMAIL' ) {
			Ext.Array.each( Optima5.Modules.Spec.RsiRecouveo.HelperCache.getEmailAll(), function(userRow) {
				if( !userRow.status_is_ext ) {
					// return ;
				}
				rootChildren.push({
					nodeId: userRow.email_adr,
					nodeType: 'entry',
					nodeKey: userRow.email_adr,
					nodeText: userRow.email_adr,
					leaf: true
				});
			}) ;
			rootNode = {
	...
	fillValues: function( values ) {
		...
		if( this.cfgParam_id && this.cfgParam_id.indexOf('ATR:')===0 ) {
			var atrId = this.cfgParam_id.substr(4),
				atrData = Optima5.Modules.Spec.RsiRecouveo.HelperCache.getAtrData(atrId) ;
			if( atrData ) {
				var atrDataMap = {} ;
				Ext.Array.each( atrData, function(drow) {
					atrDataMap[drow.id] = drow.text ;
				}) ;
			}
			allowed = true ;
		}
		...
		Ext.Array.each(values, function(value) {
			...
			rootChildren.push({
				...
				nodeId: value,
				nodeKey: value,
				nodeText: ( (atrDataMap&&atrDataMap[value]) || value )
			}) ;
		}) ;
		this.setRootNode({
			root: true,
