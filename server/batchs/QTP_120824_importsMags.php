...
$query = "select treenode_key,treenode_field_value_link from store_bible_tree_field where bible_code='STORE' AND treenode_field_code='STOREGROUPPRODS'" ;
$res = $_opDB->query($query) ;
while( ($arr = $_opDB->fetch_assoc($res)) != FALSE )
{
	$TMP_cache_STOREGROUPPRODS[$arr['treenode_key']] = $arr['treenode_field_value_link'] ;
}
