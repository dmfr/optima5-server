...
function specRsiRecouveo_report_getValuesDescNew() {
	return array(
		/*
		array(
...
function specRsiRecouveo_report_getTiles($post_data) {
	...
	foreach( specRsiRecouveo_report_getValuesDescNew() as $value_desc ) {
		$row = array(
		...
		if( $value_desc['subvalues'] ) {
			foreach( $value_desc['subvalues'] as $subvalue_desc ) {
				$ttmp = specRsiRecouveo_report_run_getValuesNew( $subvalue_desc['reportval_id'], $dates_main, $p_filters, NULL ) ;
				$value_main = reset($ttmp) ;
				$ttmp = specRsiRecouveo_report_run_getValuesNew( $subvalue_desc['reportval_id'], $dates_eval, $p_filters, NULL ) ;
				...
				$ttmp = specRsiRecouveo_report_run_getValuesNew( $value_desc['reportval_id'], $dates_main, $p_filters, NULL ) ;
				$value_main = reset($ttmp) ;
				$ttmp = specRsiRecouveo_report_run_getValuesNew( $value_desc['reportval_id'], $dates_eval, $p_filters, NULL ) ;
				$value_eval = reset($ttmp) ;
			$row['components'][] = array(
				'reportval_id' => $value_desc['reportval_id'],
				'reportval_txt' => $value_desc['reportval_txt'],
	...
	if ($p_axes['timebreak_is_on'] && count($p_vals) > 1 && !$p_axes['groupby_is_on']){
		...
		foreach( $cols as $col ) {
			foreach ($temp as $key=>$tmp){
				...
				$map_grouper_val = specRsiRecouveo_report_run_getValuesNew($tmp['reportval_id'],$dates,$p_filters,$grouper) ;
...
function specRsiRecouveo_report_run_getValuesNew( $reportval_id, $dates, $filters, $grouper )
{
	global $_opDB;
